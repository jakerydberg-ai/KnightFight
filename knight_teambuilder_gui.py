...
class TeamBuilderApp:
    ...
    def setup_styles(self):
        ...
        style.configure(
            "TLabel", background="#1E1E1E", foreground="#E0E0E0", font=self.label_font
        )
        style.configure(
            "TButton",
            background="#4A4A4A",
        ...
        style.map(
            "TButton", background=[("active", "#6E6E6E"), ("disabled", "#3A3A3A")]
        )
        ...
        style.map(
            ...
            indicatorcolor=[("selected", "#D4AF37")],
        )
    ...
    def create_main_layout(self):
        ...
        self.load_button = ttk.Button(
            button_frame, text="Load Warband", command=self.load_team
        )
    ...
    def show_initial_message(self):
        ...
        ttk.Label(
            self.right_frame, text="Your Warband is empty.", style="Title.TLabel"
        ).pack(pady=20)
        self.add_knight_button = ttk.Button(
            self.right_frame, text="Recruit a Knight", command=self.show_knight_builder
        )
    ...
    def show_knight_builder(self):
        ...
        ttk.Label(
            self.right_frame, text="Recruit a New Knight", style="Title.TLabel"
        ).pack(pady=10)
    ...
    def update_builder_ui(self, event=None):
        ...
        self.points_label = ttk.Label(
            stats_frame, text=f"Points Remaining: {self.points_remaining}"
        )
        ...
        for stat, value in self.base_stats.items():
            ...
            plus_button.bind(
                "<ButtonPress-1>", lambda e, s=stat, d=1: self.start_stat_change(s, d)
            )
        ...
        faction_abilities = [
            a for a in ALL_ABILITIES.values() if a.faction == template.faction
        ]
        generic_abilities = [
            a for a in ALL_ABILITIES.values() if a.faction == "Generic"
        ]
        self.available_abilities = sorted(
            faction_abilities, key=lambda a: a.name
        ) + sorted(generic_abilities, key=lambda a: a.name)

        self.ability_var = tk.StringVar()
        ability_menu = ttk.Combobox(
            ability_frame,
            textvariable=self.ability_var,
            values=[a.name for a in self.available_abilities],
            state="readonly",
        )
        ability_menu.pack(pady=5)
        ability_menu.bind("<<ComboboxSelected>>", self.update_ability_description)
        self.ability_desc_label = ttk.Label(
            ability_frame, text="", wraplength=200, justify=tk.LEFT
        )
        ...
        available_moves = sorted(
            all_possible_moves, key=lambda m: (m.faction == "Generic", m.name)
        )
        ...
        for move in available_moves:
            ...
            cb = ttk.Checkbutton(
                moves_frame,
                text=f"{move.name} ({move.faction})",
                variable=var,
                command=self.check_move_count,
                style="TCheckbutton",
            )
        ...
        self.finalize_button = ttk.Button(
            self.builder_content_frame,
            text="Add Knight to Warband",
            command=self.finalize_knight,
            style="Gold.TButton",
        )
    ...
    def start_stat_change(self, stat, delta):
        ...
        self.stat_change_job = self.root.after(
            400, lambda: self.continuous_stat_change(stat, delta)
        )

    def continuous_stat_change(self, stat, delta):
        self.change_stat(stat, delta)
        self.stat_change_job = self.root.after(
            75, lambda: self.continuous_stat_change(stat, delta)
        )
    ...
    def finalize_knight(self):
        ...
        if len(self.team) == 6:
            ...
            messagebox.showinfo(
                "Warband Complete", "Your warband is full. You can now save your team."
            )

    def update_team_display(self):
        self.team_display.delete(0, tk.END)
        for knight in self.team:
            self.team_display.insert(
                tk.END, f" {knight['custom_name']} ({knight['template']})"
            )

    def save_team(self):
        if len(self.team) < 6:
            messagebox.showwarning(
                "Incomplete Team", "You must have 6 knights in your warband to save."
            )
            return
        ...
        try:
            ...
            messagebox.showinfo(
                "Success", f"Team saved to {os.path.basename(filepath)}"
            )
    ...
    def load_team(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error Loading File", f"Could not load or parse the team file: {e}"
            )
